    ...
    this.logement.periodesDeLocation.forEach((periode) => {
      ...
      if (dateFin >= dateDebutEffective && dateDebut <= dateFinAnnee) {
        if (periode.typeDeLocation === TypeDeLocation.JOURNALIERE) {
          ...
          // Pour mensuel : calculer le nombre de mois dans l'année
          ...
          // Calculer le nombre de mois complets dans la période effective
          ...
          const nombreMois = moisFin - moisDebut + 1;

          // Le tarif représente le loyer mensuel
          const revenu = periode.tarif * nombreMois;
          revenus += revenu;
        }
      }
    });
